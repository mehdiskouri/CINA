...
async def run_query(client: httpx.AsyncClient, url: str, query: str) -> dict[str, float]:
    ...
    for raw_block in resp.text.split("\n\n"):
...
def _echo(message: str) -> None:
    """Write progress output to stdout for benchmark execution."""
    sys.stdout.write(f"{message}\n")


async def main() -> None:
    """Execute latency benchmark and persist markdown summary."""
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(120.0)) as client:
        for i, query in enumerate(queries):
            for run in range(args.runs):
                _echo(f"[{i * args.runs + run + 1}/{len(queries) * args.runs}] {query[:50]}...")
                try:
    ...
    if not all_metrics:
        _echo("No successful queries. Exiting.")
        return
    ...
    bench_path = Path("docs/benchmarks/query_latency.md")
    bench_path.parent.mkdir(parents=True, exist_ok=True)
    with bench_path.open("w", encoding="utf-8") as f:  # noqa: ASYNC230
    ...
    _echo(f"Pre-LLM p95: {p95_pre_llm:.1f}ms")
    _echo(f"Total p95: {p95_total:.1f}ms")
