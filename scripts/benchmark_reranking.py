...
if TYPE_CHECKING:
    from cina.config.schema import ServingConfigModel
...
class _BenchmarkDeps:
    cfg: ServingConfigModel
...
async def _collect_query_metrics(
    ...
    *,
    ...
    """Collect quality and latency metrics for one query across rerank configs."""
    embedding = await deps.embedder.embed(query)
    vector_results = await deps.vsearcher.search(embedding, top_k=deps.cfg.search.vector_top_k)
    bm25_results = await deps.bm25searcher.search(query, top_k=deps.cfg.search.bm25_top_k)
    fused = reciprocal_rank_fusion(vector_results, bm25_results, k=deps.cfg.search.rrf_k)
    candidates = fused[: deps.cfg.rerank.candidates]

    for config_name, reranker in deps.configs.items():
        start = time.perf_counter()
        ranked = (
            await reranker.rerank(query, candidates) if reranker is not None else candidates[:10]
        )
        elapsed_ms = (time.perf_counter() - start) * 1000

        scores = [_proxy_relevance(query, result.content) for result in ranked[:10]]
        deps.all_metrics[config_name]["ndcg"].append(_ndcg_at_k(scores))
        deps.all_metrics[config_name]["precision"].append(_precision_at_k(scores))
        deps.all_metrics[config_name]["latency_ms"].append(elapsed_ms)


def _write_adr(all_metrics: dict[str, dict[str, list[float]]]) -> None:
    """Write ADR markdown summarizing reranking benchmark outcomes."""
    adr_path = Path("docs/adr/ADR-003-reranking-approach.md")
...
async def run_benchmark() -> None:
    """Run reranking quality benchmark and write ADR markdown report."""
    clear_config_cache()
    pool = await get_pool()
    ...
    for i, query in enumerate(QUERIES):
        ...
        await _collect_query_metrics(query, deps=deps)

    await close_pool()
    _write_adr(all_metrics)
