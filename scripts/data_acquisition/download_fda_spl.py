...
def fetch_page(client: httpx.Client, page: int, page_size: int) -> list[dict]:
        ...
        while downloaded < limit:
            ...
            for row in rows:
                ...
                if download_spl(client, setid, out_dir):
                    downloaded += 1
                    if downloaded % 100 == 0:
                        print(f"downloaded={downloaded} page={page}")
                if downloaded >= limit:
                    break

            page += 1

    print(f"DONE fda_spl downloaded={downloaded} out={out_dir}")
