...
cina_ingestion_chunks_created_total = Counter(
    "cina_ingestion_chunks_created_total", "Chunks created"
)
cina_ingestion_embedding_latency_seconds = Histogram(
    "cina_ingestion_embedding_latency_seconds", "Embedding latency seconds"
)
cina_ingestion_embedding_batch_size = Histogram(
    "cina_ingestion_embedding_batch_size", "Embedding batch size"
)
cina_ingestion_errors_total = Counter(
    "cina_ingestion_errors_total", "Ingestion errors", ["stage", "error_type"]
)
cina_ingestion_queue_depth = Gauge("cina_ingestion_queue_depth", "Ingestion queue depth")

# Query serving
cina_query_latency_seconds = Histogram(
    "cina_query_latency_seconds", "Query latency by stage", ["stage"]
)
...
cina_cache_requests_total = Counter(
    "cina_cache_requests_total", "Semantic cache hits/misses", ["result"]
)
cina_provider_request_total = Counter(
    "cina_provider_request_total", "Provider requests", ["provider", "status"]
)
